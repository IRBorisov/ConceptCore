...
UTastNormalize::UTastNormalize() {
	...
	env.AddAST("F2", R"(F2:==[a \in B(X1), b \in B(X2)] (a, b))");
	env.AddAST("F3", R"(F3:==[a \in B(X1)] { t \in a | t=t})");
}
...
TEST_F(UTastNormalize, EnumDeclaration) {
	ExpectAST(R"(\A a,b \in X1 a=b)", "[\xE2\x88\x80[a][X1][\xE2\x88\x80[b][X1][=[a][b]]]]");
	ExpectAST(R"(\E a,b \in X1 a=b)", "[\xE2\x88\x83[a][X1][\xE2\x88\x83[b][X1][=[a][b]]]]");
	ExpectAST(R"(\E a,b,c \in X1 a=b)", "[\xE2\x88\x83[a][X1][\xE2\x88\x83[b][X1][\xE2\x88\x83[c][X1][=[a][b]]]]]");
}

TEST_F(UTastNormalize, Quantor) {
	ExpectAST(R"(\A (a,b) \in S1 a=b)", "[\xE2\x88\x80[@ab][S1][=[pr1[@ab]][pr2[@ab]]]]");
	ExpectAST(R"(\E (a,b) \in S1 a=b)", "[\xE2\x88\x83[@ab][S1][=[pr1[@ab]][pr2[@ab]]]]");
	ExpectAST(R"(\E (a,b,c) \in S1 (a=b && b=c))", 
...
TEST_F(UTastNormalize,TermTupleDeclaration) {
	ExpectAST(R"(D{(a,b) \in S1 | a=b})", "[DECLARATIVE[@ab][S1][=[pr1[@ab]][pr2[@ab]]]]");
	ExpectAST(R"(D{(a,b,c) \in S1 | a=b && b=c})", 
...
TEST_F(UTastNormalize, RecrusionTupleDeclaration) {
	ExpectAST(R"(R{(a,b) := S1 | (b,a)})", "[REC_SHORT[@ab][S1][TUPLE[pr2[@ab]][pr1[@ab]]]]");
	ExpectAST(R"(R{(a,b) := S1 | 1=1 | (b,a)})", "[REC_FULL[@ab][S1][=[1][1]][TUPLE[pr2[@ab]][pr1[@ab]]]]");
}
...
TEST_F(UTastNormalize, Functions) {
	...
	ExpectAST(R"(\A t \in X1 F3[{t}]=t)", "[\xE2\x88\x80[t][X1][=[DECLARATIVE[__var1][SET[t]][=[__var1][__var1]]][t]]]");
}